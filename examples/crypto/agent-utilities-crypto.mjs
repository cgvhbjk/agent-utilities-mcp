...
function paymentNetwork(network) {
  return Object.hasOwn(PAYMENT_NETWORKS, network) ? PAYMENT_NETWORKS[network] : void 0;
}
...
// node_modules/@circle-fin/x402-batching/dist/client/index.mjs
...
function warnHookFailure(component, phase, label, error) {
  const detail = error instanceof Error ? error.message : String(error);
  console.warn(`[x402] ${component} ${phase} hook threw (${label}): ${detail}`);
}
...
var BatchEvmScheme = class {
  ...
  afterPaymentCreationHooks = [];
  onPaymentCreationFailureHooks = [];
  paymentResponseHooks = [];
  /**
   * Register a hook that runs before payment payload creation. Returning
   * `{ abort: true, reason }` causes `createPaymentPayload` to throw
   * `Error("Payment creation aborted: <reason>")`.
   */
  ...
  /**
   * Register a hook that runs after a successful payment payload creation.
   */
  ...
  /**
   * Register a hook that runs when payload creation throws. Return
   * `{ recovered: true, payload }` to swallow the error and use the
   * provided payload as the result instead.
   */
  ...
  /**
   * Register a hook that runs after the buyer's paid HTTP request completes.
   * Hooks fire when {@link dispatchPaymentResponse} is called by the
   * transport. Return `{ recovered: true }` to instruct the transport to
   * retry with a fresh payload.
   */
  ...
  /**
   * Fire all registered `onPaymentResponse` hooks in order. Returns
   * `{ recovered: true }` if any hook signals recovery (first wins).
   *
   * Transport wrappers should call this after the paid HTTP request settles
   * — see `x402HTTPClient.processPaymentResult` in the standard SDK.
   */
  async dispatchPaymentResponse(context) {
    for (const [index, hook] of this.paymentResponseHooks.entries()) {
      try {
  ...
  async createPaymentPayload(x402Version2, paymentRequirements) {
    const context = {
      paymentRequired: this.synthesizePaymentRequired(x402Version2, paymentRequirements),
    ...
    try {
      ...
      const authorization = {
        ...
        // 10 minutes before
        // Clamp seller-published timeouts so the signed authorization is always
      ...
      for (const [index, hook] of this.afterPaymentCreationHooks.entries()) {
        try {
      ...
      for (const hook of this.onPaymentCreationFailureHooks) {
        ...
        if (result && "recovered" in result && result.recovered) {
          return {
            x402Version: result.payload.x402Version,
  ...
  /**
   * When `createPaymentPayload` is called directly (not through an x402Client),
   * we don't have a real PaymentRequired response. Synthesize a minimal one
   * so the hook context still carries the standard SDK's shape.
   */
  synthesizePaymentRequired(x402Version2, requirements) {
    return {
      x402Version: x402Version2,
...
var arcMainnet = arc ?? defineChain({
  id: 5042,
  name: "Arc",
...
var arcTestnetChain = arcTestnet ?? defineChain({
  id: 5042002,
  name: "Arc Testnet",
  nativeCurrency: { decimals: 18, name: "USDC", symbol: "USDC" },
  rpcUrls: {
...
var CHAIN_CONFIGS = {
  ...
  arc: {
    // No `rpcUrl`: Arc mainnet has no public RPC until ~2026-06-22. Partners must
    // pass their private RPC via GatewayClientConfig.rpcUrl (enforced in the
    // GatewayClient constructor).
    chain: arcMainnet,
    domain: GATEWAY_DOMAINS.arc,
    usdc: "0x3600000000000000000000000000000000000000",
    gatewayWallet: MAINNET_GATEWAY_WALLET,
    gatewayMinter: MAINNET_GATEWAY_MINTER
  }
...
// node_modules/@x402/core/dist/esm/chunk-RAWLCYSQ.mjs
...
async function digest(value) {
  return b64(new Uint8Array(await crypto.subtle.digest("SHA-256", encoder3.encode(value))));
}
async function commitmentMessage(token, payload) {
  return `Agent Utilities request binding v1
Quote: ${token}
Payment SHA256: ${await digest(canonical(payload))}`;
}

// src/client.ts
async function prepareCall(options) {
  const config = paymentNetwork(options.expectedNetwork);
  if (!config) throw Error("Unsupported payment network.");
  const url = new URL(options.url);
  if (url.protocol !== "https:" && !(url.protocol === "http:" && ["127.0.0.1", "localhost"].includes(url.hostname))) throw Error("Use HTTPS or a loopback development URL.");
  if (url.username || url.password) throw Error("Credentials in URLs are not supported.");
  if (!Number.isSafeInteger(options.maxMicroUsdc) || options.maxMicroUsdc < 1) throw Error("Set an integer per-call spending limit.");
  const body = JSON.stringify(options.body);
  const response = await (options.fetcher || fetch)(url.href, { method: "POST", headers: { "Content-Type": "application/json" }, body, redirect: "error", signal: AbortSignal.timeout(15e3) });
  if (response.status !== 402) throw Error(`Expected a quote; received HTTP ${response.status}. No authorization created.`);
  const required = await response.json();
  const info = required.extensions?.["agent-utilities-request-binding"]?.info;
  const expires = typeof info?.expiresAt === "string" ? Date.parse(info.expiresAt) : NaN;
  if (required.x402Version !== 2 || required.accepts?.length !== 1 || !info || info.version !== 1 || typeof info.quote !== "string" || !Number.isFinite(expires) || expires <= Date.now()) throw Error("Unsupported or expired quote.");
  const terms = required.accepts[0];
  if (typeof terms.payTo === "string" && terms.payTo.toLowerCase() === options.account.address.toLowerCase()) throw Error("Gateway cannot pay the same wallet that is receiving. Use a different paying wallet.");
  if (!/^\d{1,12}$/.test(terms.amount) || BigInt(terms.amount) > BigInt(options.maxMicroUsdc) || BigInt(terms.amount) < 1n || terms.payTo.toLowerCase() !== options.expectedRecipient.toLowerCase() || terms.network !== options.expectedNetwork || terms.asset.toLowerCase() !== config.usdc || terms.extra?.verifyingContract?.toString().toLowerCase() !== config.gateway || terms.scheme !== "exact" || terms.extra?.name !== "GatewayWalletBatched" || terms.extra?.version !== "1" || !Number.isSafeInteger(terms.maxTimeoutSeconds) || terms.maxTimeoutSeconds < 1 || terms.maxTimeoutSeconds > 691200) throw Error("Quote exceeds the approved budget, recipient, network or contract.");
  if (!options.account.signMessage) throw Error("A signing account is required.");
  const partial = await new BatchEvmScheme(options.account).createPaymentPayload(2, terms);
  const payload = { x402Version: partial.x402Version, payload: { ...partial.payload }, accepted: terms, resource: required.resource };
  const binding = await options.account.signMessage({ message: await commitmentMessage(info.quote, payload) });
  return { url: url.href, body, quoteExpiresAt: info.expiresAt, headers: { "Content-Type": "application/json", "PAYMENT-SIGNATURE": encodePaymentSignatureHeader(payload), "X-Quote": info.quote, "X-Quote-Signature": binding } };
}
async function sendPrepared(call, fetcher = fetch) {
  return fetcher(call.url, { method: "POST", headers: call.headers, body: call.body, redirect: "error", signal: AbortSignal.timeout(25e3) });
}
export {
  prepareCall,
  privateKeyToAccount,
