...
Leave the plugin's `AGENT_UTILITIES_API_KEY` variable empty for free tool discovery. For paid execution, configure an API key privately through the client's plugin settings. Never put the key into the repository or a shared project file, and never give the plugin your account recovery credential. Keep tool approvals enabled. Requests send inputs to Agent Utilities under its [data policy](https://agent-utilities.agent-utilities.workers.dev/policies?utm_source=github).
